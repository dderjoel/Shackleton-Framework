...
void gi_llvm_pass_randomizeobject(object_gi_llvm_pass_str *o) {
  ...
  uint32_t new_index0 =
      (uint32_t)(num_valid_values0 * (rand() / (RAND_MAX + 1.0)));
  GI_LLVM_PASS_PATCH_TYPE_MY_PATCH_TYPE_INDEX(o) = new_index0;
  GI_LLVM_PASS_PATCH_TYPE_MY_PATCH_TYPE(o) =
      GI_LLVM_PASS_PATCH_TYPE_VALID_VALUES(o)[new_index0];
  ...
  uint32_t new_index1 =
      (uint32_t)(num_valid_values1 * (rand() / (RAND_MAX + 1.0)));
  GI_LLVM_PASS_NEW_PASS_MY_NEW_PASS_INDEX(o) = new_index1;
  GI_LLVM_PASS_NEW_PASS_MY_NEW_PASS(o) =
      GI_LLVM_PASS_NEW_PASS_VALID_VALUES(o)[new_index1];
}

void gi_llvm_pass_setobject(object_gi_llvm_pass_str *o, char *pass) { return; }
...
void *gi_llvm_pass_copyobject(object_gi_llvm_pass_str *o) {
  ...
  GI_LLVM_PASS_PATCH_TYPE_MY_PATCH_TYPE_INDEX(c) =
      GI_LLVM_PASS_PATCH_TYPE_MY_PATCH_TYPE_INDEX(o);
  GI_LLVM_PASS_PATCH_TYPE_MY_PATCH_TYPE(c) =
      GI_LLVM_PASS_PATCH_TYPE_VALID_VALUES(
          c)[GI_LLVM_PASS_PATCH_TYPE_MY_PATCH_TYPE_INDEX(o)];
  ...
  GI_LLVM_PASS_NEW_PASS_MY_NEW_PASS_INDEX(c) =
      GI_LLVM_PASS_NEW_PASS_MY_NEW_PASS_INDEX(o);
  GI_LLVM_PASS_NEW_PASS_MY_NEW_PASS(c) = GI_LLVM_PASS_NEW_PASS_VALID_VALUES(
      c)[GI_LLVM_PASS_NEW_PASS_MY_NEW_PASS_INDEX(o)];
...
bool gi_llvm_pass_compareobject(object_gi_llvm_pass_str *o1,
  ...
  if (GI_LLVM_PASS_PATCH_TYPE_MY_PATCH_TYPE_INDEX(o1) !=
      GI_LLVM_PASS_PATCH_TYPE_MY_PATCH_TYPE_INDEX(o2)) {
    return false;
  }
  if (GI_LLVM_PASS_NEW_PASS_MY_NEW_PASS_INDEX(o1) !=
      GI_LLVM_PASS_NEW_PASS_MY_NEW_PASS_INDEX(o2)) {
    return false;
  }
  if (fabs(GI_LLVM_PASS_LOCATION_MY_LOCATION(o1) -
           GI_LLVM_PASS_LOCATION_MY_LOCATION(o2)) > 0.001) {
    return false;
  }
  return true;
}
