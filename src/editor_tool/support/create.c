...
void create_macros(char *name, char *macros, cJSON *root) {
  ...
  for (uint32_t i = 0; i < params_count; i++) {
    ...
    char *param_macro_name =
        cJSON_GetObjectItem(param, "macro_name")->valuestring;
    char *param_has_valid_values =
        cJSON_GetObjectItem(param, "has_valid_values")->valuestring;
    itoa(i, curr_num, 10);
...
void create_valid_value_methods(char *name, char *methods, cJSON *root) {
  ...
  for (uint32_t j = 0; j < params_count; j++) {
    ...
    char *param_name = cJSON_GetObjectItem(new_param, "name")->valuestring;
    char *param_type = cJSON_GetObjectItem(new_param, "type")->valuestring;
    char *param_constrained =
        cJSON_GetObjectItem(new_param, "has_valid_values")->valuestring;
    ...
    if (strcmp(param_constrained, "true") == 0) {
      ...
      char full_macro_name[200];
      char *macro_name =
          cJSON_GetObjectItem(new_param, "macro_name")->valuestring;
      strupr(macro_name);
      ...
      for (uint32_t k = 0; k < num_valid_values; k++) {
        ...
        if (strcmp(param_type, "uint32_t") == 0 ||
            strcmp(param_type, "int") == 0) {

          cJSON *single_value = cJSON_GetArrayItem(valid_values, k);

          uint32_t single_valueint =
              cJSON_GetObjectItem(single_value, "value")->valueint;
