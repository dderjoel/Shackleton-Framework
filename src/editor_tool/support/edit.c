...
bool edit_modules_c_read(char *name, char *content) {
  ...
  while ((read = getline(&line, &len, modules)) != -1) {
    ...
    if (done) {
      ...
      // unless, you're at the maxtype line, this is updated with the new
      // incremented value
    ...
    // inside the object functions table array
      ...
      // found terminator, add new type before adding terminator and set done to
      // true
    ...
    // start of needed array, need to look for where to add so set in_array to
    // true
    else if (strstr(line, "{") != NULL) {
