 ...
 *  double* fitness_values - fitness values for every individual in the current
 * generation osaka_object_typ ot - the object type that is being used for this
 * run
 *
 * RETURN
 *
...
void evolution_cache_generation_old(bool cache, char *main_folder, uint32_t gen,
                                    uint32_t pop_size, node_str **curr_gen,
                                    bool vis, char *file, char **src_files,
                                    uint32_t num_src_files,
                                    double *fitness_values, osaka_object_typ ot,
                                    double *track_fitness, int offset) {
  // added 6/2/2021
  // void evolution_cache_generation(char* main_folder, uint32_t gen, uint32_t
  // pop_size, node_str** curr_gen, bool vis, char* file, char** src_files,
  // uint32_t num_src_files, double* fitness_values, osaka_object_typ ot) {
  if (!cache) {
    return;
  }
...
void select_elites(int pop_size, int num_elites, double *fitness_values,
                   int *current_gen_id, int *elite_indx, int *elite_id) {
  for (int i = 0; i < pop_size; i++) {
    printf("%s%d: %lf%s", i == 0 ? "pop_fitness=[" : "", current_gen_id[i],
           fitness_values[i], i == pop_size - 1 ? "]\n" : ", ");
  }
  ...
  for (int k = 0; k < pop_size; k++) {
    if (is_in_list(current_gen_id[k], elite_id, num_elites)) {
      continue; // to avoid populating elites with the same sequence (happens
                // when sequence repeats in population)
    }
    for (int e = num_elites - 1; e >= 0; e--) {
      if (elite_indx[e] == -1 ||
          fitness_values[k] < fitness_values[elite_indx[e]]) {
        if (e != 0) {
          elite_indx[e] = elite_indx[e - 1];
          elite_id[e] =
              elite_indx[e] == -1 ? -1 : current_gen_id[elite_indx[e - 1]];
        } else {
          elite_indx[e] = k;
...
void evolution_cache_gen(bool cache, char *main_folder,
                         node_str **current_generation, double *fitness_values,
                         int *current_gen_id, double *track_fitness,
  ...
  evolution_log_to_summary(cache, main_folder, fitness_values, current_gen_id,
                           winner_fitness, pop_size, num_gens, g, offset);
}
...
void evolution_log_to_summary(bool cache, char *main_folder,
                              ...
                              double winner_fitness, int pop_size, int num_gens,
                              int g, int offset) {
  // print out and export the ID and fitness information to summary file
  if (!cache) {
    return;
  ...
  fprintf(track_fitness_file_ptr, "%lf%s", winner_fitness,
          g == num_gens - 1 ? "\n" : ",");
  fclose(track_fitness_file_ptr);

  // log to /main_folder/test_compare.csv
  if (g == -1) { // if initial generation, no need to redo baseline evaluation,
                 // just add best fitness to end of line
  ...
  // track all id changes over generation, and a separate file recording their
  // fitness set up before constructing the all_indiv table so this is no longer
  // needed
  /*for (int k = 0; k < pop_size; k++) {
      if (cache) {
          ...
          if (k == 0) {
              fprintf(track_id_file_ptr, "%s%d, %d%s", g==-1?"initial":"gen",
  g+1, current_gen_id[k], k<pop_size-1?",":"\n");
              fprintf(track_all_fitness_file_ptr, "%s%d, %d%s",
  g==-1?"initial":"gen", g+1, fitness_values[k], k<pop_size-1?",":"\n"); } else
  { fprintf(track_id_file_ptr, "%d%s", current_gen_id[k],
  k<pop_size-1?",":"\n"); fprintf(track_all_fitness_file_ptr, "%lf%s",
  fitness_values[k], k<pop_size-1?",":"\n");
          }
...
void print_elites(int num_elites, int *elite_indx, double *fitness_values,
                  int *elite_id, node_str **current_gen) {
  printf("Created elite population of size %d: ", num_elites);
  if (num_elites > 0) {
    printf("elite_id=[");
  }
  for (int e = 0; e < num_elites; e++) {
    printf("%d (len=%d)%s", elite_id[e], osaka_listlength(current_gen[e]),
           (e == num_elites - 1) ? "], fitness=([" : ", ");
  }
  for (int e = 0; e < num_elites; e++) {
    printf("%lf%s", fitness_values[elite_indx[e]],
           (e == num_elites - 1) ? "])\n" : ", ");
  }
}
...
void print_random(int num_new_random, int num_elites, int *current_gen_id) {
  ...
  for (int e = 0; e < num_new_random; e++) {
    printf("%d%s", current_gen_id[num_elites + e],
           (e == num_new_random - 1) ? "]\n" : ", ");
  }
}

void create_elites(int num_elites, int *elite_indx, node_str **copy_gen,
                   node_str **current_generation, int *copy_gen_id,
                   int *current_gen_id, double *fitness_values) {
  for (uint32_t p = 0; p < num_elites; p++) {
    int elite_ind = elite_indx[p];
    node_str *elite = osaka_copylist(copy_gen[elite_ind]);
    // free individuals from current population to make room for elite
    // individuals
...
void create_randoms(int num_elites, int num_new_random, int *max_id_ptr,
                    node_str **current_generation, int *current_gen_id,
                    int indiv_size, osaka_object_typ ot,
                    DataNode ***all_indiv_ptr, int *hash_cap_ptr) {
  ...
  for (uint32_t p = num_elites; p < num_elites + num_new_random; p++) {
    ...
    new_allele_id =
        node_add(new_seq, max_id_ptr, hash_cap_ptr, all_indiv_ptr, ot);
    // fitness_top(offsprings[i], vis, test_file, src_files, num_src_files,
    // false, NULL, cache_id, (*all_indiv_ptr)[ofs_id[i]], num_runs, g,
    // fitness_with_var);
    ...
    // p-num_elites+1, num_elites, new_allele_id);
...
void select_parents(uint32_t *contestant1_ind, uint32_t *contestant2_ind,
                    node_str **copy_gen, double *fitness_values, int copy_size,
                    int tourn_size, bool vis) {
  // printf("inside select parents\n");
  uint32_t c1 = selection_tournament(copy_gen, fitness_values, NULL, copy_size,
                                     tourn_size, vis);
  uint32_t c2 = selection_tournament(copy_gen, fitness_values, NULL, copy_size,
                                     tourn_size, vis);

  // contestants cannot be the same individual, the indices must be different
  while (c1 == c2) {
    c2 = selection_tournament(copy_gen, fitness_values, NULL, copy_size,
                              tourn_size, vis);
  }

  // swap if contestant1 comes before contestant2, swap so deletion order is
  // correct
  if (c1 < c2) {
  ...
  // printf("parents selected: c1: ind=%d, fitness=%lf, c2: ind=%d,
...
void generate_offspring(int parent1_ind, int parent2_ind, node_str **copy_gen,
                        int *copy_gen_id, int num_offspring,
                        node_str **offsprings, bool *ofs_change, int *ofs_id,
                        uint32_t cross_perc, uint32_t mut_perc, bool vis,
                        int *max_id_ptr, int *hash_cap_ptr,
                        DataNode ***all_indiv_ptr, osaka_object_typ ot) {
  for (int i = 0; i < num_offspring; i++) {
    ...
    if (i % 2 == 0) {
      ...
      if (i == num_offspring - 1) {
        ...
        bool temp_change;
        genetic_operators(temp, offsprings[i], &temp_change, &ofs_change[i],
                          cross_perc, mut_perc, vis);
        if (ofs_change[i]) {
          ofs_id[i] = node_add(offsprings[i], max_id_ptr, hash_cap_ptr,
                               all_indiv_ptr, ot);
        }
      ...
      if (i != 1) {
        genetic_operators(offsprings[i - 1], offsprings[i], &ofs_change[i - 1],
                          &ofs_change[i], cross_perc, mut_perc, vis);
        // printf("ofs_change[%d]=%s, ofs_change[%d]=%s\n", i-1,
        // ofs_change[i-1]?"true":"false", i, ofs_change[i]?"true":"false");
        if (ofs_change[i - 1]) {
          ofs_id[i - 1] = node_add(offsprings[i - 1], max_id_ptr, hash_cap_ptr,
                                   all_indiv_ptr, ot);
        }
        if (ofs_change[i]) {
          ofs_id[i] = node_add(offsprings[i], max_id_ptr, hash_cap_ptr,
...
void genetic_operators(node_str *contestant1, node_str *contestant2,
                       bool *c1_change, bool *c2_change, uint32_t cross_perc,
                       uint32_t mut_perc, bool vis) {
  ...
  // random numbers are used to decide if the crossover or mutation operators
  // will be used with a certain probability
  ...
  if (temp_mutation1 <= mut_perc) {
    ...
    uint32_t random =
        (uint32_t)(indiv_size_1 * (rand() / (RAND_MAX + 1.0))) + 1;
  ...
  if (temp_mutation2 <= mut_perc) {
    ...
    uint32_t random =
        (uint32_t)(indiv_size_2 * (rand() / (RAND_MAX + 1.0))) + 1;
...
void select_offspring(node_str **best, int *best_id, node_str **offsprings,
                      bool *ofs_change, int *ofs_id, int num_offspring,
                      int parent1_ind, int parent2_ind, double *fitness_values,
                      bool vis, char *test_file, char **src_files,
                      uint32_t num_src_files, bool cache, char *cache_file,
                      const char *cache_id, int g, DataNode ***all_indiv_ptr,
                      uint32_t num_runs, bool fitness_with_var) {
  // printf("\nInside select_offspring, calculating offspring fitness,
  // num_offspring=%d:\n", num_offspring);

  double ofs_fitness[num_offspring];
  for (int i = 0; i < num_offspring; i++) {
    ofs_fitness[i] = fitness_top(
        offsprings[i], vis, test_file, src_files, num_src_files, false, NULL,
        cache_id, (*all_indiv_ptr)[ofs_id[i]], num_runs, g, fitness_with_var);
  ...
  ofs_change[min1_ind]?"true":"false"); printf("indiv 2: ofs_ind=%d,
  fitness=%lf, change=%s\n", min2_ind, min2_fit,
  ofs_change[min2_ind]?"true":"false");
  */
...
void update_generation(node_str *contestant1, node_str *contestant2, int c1_id,
                       int c2_id, node_str **current_generation,
                       int *current_gen_id, int pop_size, int num_elites,
                       int num_new_random, int p) {
  ...
  generate_free_individual(
      current_generation[num_elites + num_new_random + p +
                         ((pop_size - num_elites - num_new_random) / 2)]);
  // printf("Freed individual #%d for contestant 2\n\n", num_elites +
  // num_new_random + p + ((pop_size-num_elites-num_new_random) / 2));
  current_generation[num_elites + num_new_random + p +
                     ((pop_size - num_elites - num_new_random) / 2)] =
      contestant2;
  current_gen_id[num_elites + num_new_random + p +
                 ((pop_size - num_elites - num_new_random) / 2)] = c2_id;

  // printf("Done updating generation\n");
}

void create_mutants(node_str **copy_gen, node_str **current_generation,
                    double *fitness_values, int *copy_gen_id,
                    int *current_gen_id, int *max_id_ptr, uint32_t tourn_size,
                    uint32_t pop_size, uint32_t cross_perc, uint32_t mut_perc,
                    uint32_t num_elites, uint32_t num_new_random,
                    uint32_t copy_size, uint32_t num_runs,
                    uint32_t num_offspring, char *file, char **src_files,
                    uint32_t num_src_files, bool vis, int g,
                    osaka_object_typ ot, bool cache, char *cache_file,
                    const char *cache_id, DataNode ***all_indiv_ptr,
                    int *hash_cap_ptr, bool fitness_with_var) {
  ...
  for (uint32_t itr = 0; itr < ((pop_size - num_elites - num_new_random) / 2);
       itr++) {
    // printf("About to fill in position %d and %d\n", num_elites +
    // num_new_random + itr, num_elites + num_new_random + itr +
    // ((pop_size-num_elites-num_new_random) / 2));
    vis_itr(vis, itr, g);
    // printf("before select_parents\n");
    select_parents(&contestant1_ind, &contestant2_ind, copy_gen, fitness_values,
                   copy_size, tourn_size, vis);
    // printf("after select_parents\n");
    // printf("before generate_offspring\n");
    generate_offspring(contestant1_ind, contestant2_ind, copy_gen, copy_gen_id,
                       num_offspring, offsprings, ofs_change, ofs_id,
                       cross_perc, mut_perc, vis, max_id_ptr, hash_cap_ptr,
                       all_indiv_ptr, ot);
    // printf("after generate_offspring\n");
    vis_print_parents(vis, offsprings);
    // printf("before select_offspring\n");
    select_offspring(ofs_best, best_id, offsprings, ofs_change, ofs_id,
                     num_offspring, contestant1_ind, contestant2_ind,
                     fitness_values, vis, file, src_files, num_src_files, cache,
                     cache_file, cache_id, g, all_indiv_ptr, num_runs,
                     fitness_with_var);
    // printf("after select_offspring\n");
    // printf("before update_generation\n");
    update_generation(ofs_best[0], ofs_best[1], best_id[0], best_id[1],
                      current_generation, current_gen_id, pop_size, num_elites,
                      num_new_random, itr);
...
void log_all_indiv_info(bool cache, DataNode **all_indiv, char *main_folder,
                        int num_runs, int max_id) {
  if (cache) {
    ...
    ;
    fprintf(indiv_info_file_ptr,
            "ID,num_eval,tot_gen,gen_#,avg_time,var,success_runs,");
...
void log_redo_basic(char *main_folder, char *file, bool cache,
                    const char *cache_id, double best_fitness,
                    uint32_t num_runs, bool fitness_with_var, int g,
  ...
  if ((g + 1) % 5 == 0) {
    ...
    fitness_redo_basic(main_folder, file, cache, temp_track, cache_id, num_runs,
...
bool check_termination(double best_fitness, double *lowest_ptr,
                       int *stale_counter_ptr, const int stale_limit) {
  ...
  if (*stale_counter_ptr >= stale_limit) {
    printf("Number of stale generations: %d reaches stale_limit=%d, evolution "
           "terminates\n",
...
int evolution_clean_up(int num_elites, node_str **current_generation,
                       uint32_t pop_size, bool vis, char *main_folder,
                       char *file, const char *cache_id, bool cache,
                       DataNode **all_indiv, int num_runs, int max_id, int g,
                       osaka_object_typ ot, double *fitness_values,
                       int *current_gen_id) {
 ...
 * EXAMPLE
 *
 * node_str* best = evolution_basic_crossover_and_mutation_with_replacement(20,
 * 50, 50, 2, 5, 25, LLVM_PASS, true);
 *
 * SIDE-EFFECT
 *
 * none
...
int evolution_basic_crossover_and_mutation_with_replacement(
    ...
    const char *cache_id, const char **levels, const int num_levels, bool gi) {
  if (vis) {
    printf("Performing basic tournament/crossover/mutation evolution with "
           "replacement --------\n\n");
  }
  ...
  double lowest =
      UINT32_MAX; // Lowest runtime encountered, for termination criteria only
  int stale_counter =
      0; // Number of generations without new low, for termination criteria only
  ...
  int max_id = 0;
  /* number of positions that generation fitness from shackleton is offset in
     the track_fitness vector because fitness for basic optimization levels and
     initial generation are recorded before them */
  ...
  int num_offspring =
      6; // HAS TO BE GREATER THAN 2: 2 offsprings will be identical to parents,
         // and num_offspring-2 new individuals
  uint32_t num_runs = 40;
  bool fitness_with_var = false;

  cache_create_new_run_folder(cache, main_folder, cache_id);
  cache_params(cache, main_folder, num_gens, pop_size, cross_perc, mut_perc,
               elite_perc, tourn_size);
  fitness_pre_cache(main_folder, file, src_files, num_src_files, ot, cache,
                    track_fitness, cache_id, num_runs, fitness_with_var, levels,
                    num_levels);

  // create the initial population
  generate_new_generation(current_generation, pop_size, indiv_size, ot, gi,
                          levels, num_levels);
  node_add_group(current_generation, current_gen_id, pop_size, &max_id,
                 &hash_cap, &all_indiv, ot);
  for (int i = 0; i < pop_size; i++) {
    printf("%s%d%s", i == 0 ? "current_gen_id=[" : "", current_gen_id[i],
           i == pop_size - 1 ? "]\n" : ", ");
  }
  ...
  for (uint32_t k = 0; k < pop_size; k++) {
    // printf("-----------------------------------------------------------------------------\n");
    // printf("Calculate initial fitness - individual #%d out of %d, ID=%d
    ...
    fitness_values[k] =
        fitness_top(current_generation[k], vis, file, src_files, num_src_files,
                    cache, cache_file, cache_id, indiv_data, num_runs,
                    generation_num, fitness_with_var); // Added 6/24/2021
    node_increment_gen(indiv_data);
    //=%lf\n", fitness_values[k]);
  }
  // if cache, record generation information
  // evolution_cache_generation(cache, main_folder, -1, pop_size,
  // current_generation, vis, file, src_files, num_src_files, fitness_values,
  // ot, track_fitness);
  // update elite list as the best N individuals in the generation
  select_elites(pop_size, num_elites, fitness_values, current_gen_id,
                elite_indx, elite_id);
  // print out and export the ID and fitness information
  evolution_cache_gen(cache, main_folder, current_generation, fitness_values,
                      current_gen_id, track_fitness, pop_size, num_gens,
                      generation_num, offset, ot);
  ...
  for (uint32_t g = 0; g < num_gens; g++) {
    ...
    create_elites(num_elites, elite_indx, copy_gen, current_generation,
                  copy_gen_id, current_gen_id, fitness_values);
    print_elites(num_elites, elite_indx, fitness_values, elite_id,
                 current_generation);
    // printf("after create_elites\n");

    // printf("before create_randoms\n");
    create_randoms(num_elites, num_new_random, &max_id, current_generation,
                   current_gen_id, indiv_size, ot, &all_indiv, &hash_cap);
    print_random(num_new_random, num_elites, current_gen_id);
    // printf("after create_randoms\n");

    // printf("before create_mutants\n");
    create_mutants(copy_gen, current_generation, fitness_values, copy_gen_id,
                   current_gen_id, &max_id, tourn_size, pop_size, cross_perc,
                   mut_perc, num_elites, num_new_random, pop_size, num_runs,
                   num_offspring, file, src_files, num_src_files, vis, g, ot,
                   cache, cache_file, cache_id, &all_indiv, &hash_cap,
                   fitness_with_var);
    ...
    for (uint32_t k = 0; k < pop_size; k++) {
      /*
      printf("-------------------------------------------------------------------------------\n");
      printf("Refresh fitness for individual %d of %d in generation %d of %d,
      ID=%d (max_id=%d)\n", k+1, pop_size, g+1, num_gens, current_gen_id[k],
      max_id);
      printf("-------------------------------------------------------------------------------\n");
      */
      ...
      indiv_data = all_indiv[current_gen_id[k]];
      fitness_values[k] =
          fitness_top(current_generation[k], vis, file, src_files,
                      num_src_files, cache, cache_file, cache_id, indiv_data,
                      num_runs, g, fitness_with_var); // Added 6/18/2021
      node_increment_gen(indiv_data);
      // printf("Fitness=%lf\n", fitness_values[k]);
    }
    select_elites(pop_size, num_elites, fitness_values, current_gen_id,
    ...
    evolution_cache_gen(cache, main_folder, current_generation, fitness_values,
                        current_gen_id, track_fitness, pop_size, num_gens, g,
                        offset, ot);
    ...
    printf("-------------------------------- End of Generation %d "
           "--------------------------------\n\n",
           g + 1);
    log_redo_basic(main_folder, file, cache, cache_id,
                   track_fitness[g + offset], num_runs, fitness_with_var, g,
                   levels, num_levels);
    // bool terminate = check_termination(track_fitness[g + offset], &lowest,
    // &stale_counter, stale_limit);
    bool terminate = false;
    if (terminate) {
      return evolution_clean_up(num_elites, current_generation, pop_size, vis,
                                main_folder, file, cache_id, cache, all_indiv,
                                num_runs, max_id, g, ot, fitness_values,
                                current_gen_id);
    }
  }
  // TODO: FIX THIS
  /*double temp_track[num_levels];
  fitness_redo_basic(main_folder, file, cache, temp_track, cache_id, num_runs,
  fitness_with_var, levels, num_levels); char test_compare[200];*/

  return evolution_clean_up(num_elites, current_generation, pop_size, vis,
                            main_folder, file, cache_id, cache, all_indiv,
                            num_runs, max_id, num_gens, ot, fitness_values,
                            current_gen_id);
}
