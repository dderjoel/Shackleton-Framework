...
// char defaultO3[86][50] = {"-licm", "-jump-threading", "-memoryssa",
// "-early-cse-memssa", "-memdep", "-tbaa", "-bdce", "-instsimplify",
// "-loop-sink", "-strip-dead-prototypes", "-aa", "-argpromotion",
// "-callsite-splitting", "-aggressive-instcombine", "-scalar-evolution",
// "-transform-warning", "-phi-values", "-early-cse", "-block-freq", "-gvn",
// "-verify", "-globaldce", "-instcombine", "-correlated-propagation",
// "-prune-eh", "-postdomtree", "-mldst-motion", "-loop-unswitch",
// "-elim-avail-extern", "-ipsccp", "-slp-vectorizer", "-branch-prob",
// "-loop-unroll", "-basicaa", "-lower-expect", "-basiccg", "-loop-distribute",
// "-sccp", "-inline", "-functionattrs", "-lazy-branch-prob", "-adce",
// "-float2int", "-alignment-from-assumptions", "-inferattrs",
...
// "-called-value-propagation", "-demanded-bits", "-div-rem-pairs",
// "-forceattrs", "-globalopt", "-indvars", "-simplifycfg", "-tti",
// "-memcpyopt", "-loop-rotate", "-targetlibinfo", "-barrier", "-lcssa",
// "-loop-deletion", "-loop-load-elim", "-sroa", "-lazy-block-freq",
// "-ee-instrument", "-loops", "-assumption-cache-tracker", "-loop-accesses",
// "-deadargelim", "-scoped-noalias", "-speculative-execution",
// "-tailcallelim"};
...
char **get_default(int num_levels, int target_default) {
  int *default_string_lengths =
      gi_llvm_default_level_string_lengths(num_levels);
  char ***default_strings =
      gi_llvm_set_default_level_strings(default_string_lengths, num_levels);
