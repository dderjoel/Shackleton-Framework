...
double node_record_data(DataNode *d, node_str *sequence, double *all_runtime,
  ...
  if (!osaka_compare(d->seq, sequence)) {
    printf("\n\nATTENTION: RECORD DATA SEQEUNCES DON'T MATCH "
           "(node_record_data)\n");
    printf("d->seq (id=%d): \n", d->seq_id);
...
double node_update_fitness(DataNode *d, bool fitness_with_var) {
  ...
  for (uint32_t i = 0; i < d->num_eval; i++) {
    ...
    // d->success_cts[i], i, d->avg_time[i], i, d->var[i]);
...
void node_add_group(node_str **gen, int *current_gen_id, uint32_t group_size,
  ...
  for (int g = 0; g < group_size; g++) {
    int new_allele_id =
        node_add(gen[g], max_id_ptr, hash_cap_ptr, all_indiv_ptr, ot);
...
void node_check_overflow_array(int new_indiv_id, int *hash_cap_ptr,
  ...
  if (new_indiv_id >= *hash_cap_ptr) {
    DataNode **temp =
        realloc(*all_indiv_ptr, 2 * (*hash_cap_ptr) * sizeof(DataNode *));
    if (!temp) {
...
void node_increment_gen(DataNode *d) { d->tot_gen += 1; }

void node_log(char *indiv_info_dir, char *file, DataNode *d) {
  FILE *file_ptr = fopen(file, "a");
  for (int g = 0; g < d->num_eval; g++) {
    fprintf(file_ptr, "%d,%d,%d,%d,%lf,%lf,%d,", d->seq_id, d->num_eval,
            d->tot_gen, d->gens[g], d->avg_time[g], d->var[g],
            d->success_cts[g]);
    double *time_arr = d->time_arrs[g];
    for (int r = 0; r < d->success_cts[g]; r++) {
      fprintf(file_ptr, "%lf%s", d->time_arrs[g][r],
              (r < (d->success_cts[g] - 1) ? "," : "\n"));
    }
...
void node_cache_llvm_pass(char *file_name, node_str *indiv, double fitness,
  ...
  strcpy(string, "");
  sprintf(string,
          "Description file for Individual (unique ID: %d)\n\nIndividual Size: "
...
void node_print(DataNode *d, int id) {
  for (int g = 0; g < d->num_eval; g++) {
    printf("%d,%d,%d,%lf,%lf,%d,", id, d->tot_gen, d->gens[g], d->avg_time[g],
           d->var[g], d->success_cts[g]);
    // double* time_arr = d->time_arrs[g];
    for (int r = 0; r < d->success_cts[g]; r++) {
      printf("%lf%s", d->time_arrs[g][r],
             (r < (d->success_cts[g] - 1) ? "," : "\n"));
    }
  }
}
